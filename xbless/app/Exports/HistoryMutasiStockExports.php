<?php

namespace App\Exports;

use DB;
use Maatwebsite\Excel\Concerns\WithMapping;
use Maatwebsite\Excel\Concerns\WithHeadings;
use Maatwebsite\Excel\Concerns\FromCollection;
use Maatwebsite\Excel\Concerns\WithColumnFormatting;
use PhpOffice\PhpSpreadsheet\Style\NumberFormat;
use PhpOffice\PhpSpreadsheet\Shared\Date;
use Illuminate\Contracts\View\View;
use Maatwebsite\Excel\Concerns\FromView;
use Maatwebsite\Excel\Concerns\ShouldAutoSize;
use App\Models\Perusahaan;
use App\Models\Gudang;
use App\Models\PerusahaanGudang;
use App\Models\Product;
use App\Models\StockAdj;
use App\Models\ReportStock;
use App\Models\StockMutasi;

class HistoryMutasiStockExports implements FromView,ShouldAutoSize
{
    
    protected $filter_perusahaan;
    protected $filter_gudang;
    protected $filter_gudang_tujuan;
    protected $tgl_start;
    protected $tgl_end;

    public function __construct($filter_perusahaan,$filter_gudang,$filter_gudang_tujuan,$tgl_start,$tgl_end)
    {
        $this->filter_perusahaan        = $filter_perusahaan;
        $this->filter_gudang            = $filter_gudang;
        $this->filter_gudang_tujuan     = $filter_gudang_tujuan;
        $this->tgl_start                = $tgl_start;
        $this->tgl_end                  = $tgl_end;
    }
 
    public function view(): View
    {
       
        $perusahaan             = Perusahaan::find($this->filter_perusahaan);
        $gudang                 = Gudang::find($this->filter_gudang);
        $gudangtujuan           = Gudang::find($this->filter_gudang_tujuan);

        $querydb = StockMutasi::select('log_stock.*','product.product_name','satuan.name');
        $querydb->join('product_perusahaan_gudang','product_perusahaan_gudang.id','log_stock.product_perusahaan_gudang_id');
        $querydb->join('product','product.id','product_perusahaan_gudang.product_id');
        $querydb->join('satuan','product.satuan_id','satuan.id');
        $querydb->orderBy('id','DESC');

        if($this->filter_perusahaan != ""){
            $querydb->where('log_stock.from_perusahaan_id',$this->filter_perusahaan);
        }else{
            $querydb->where('log_stock.from_perusahaan_id',0);
        }
        if($this->filter_gudang != ""){
            $querydb->where('log_stock.from_gudang_id',$this->filter_gudang);
        }else{
            $querydb->where('log_stock.from_gudang_id',0);
        }
        if($this->filter_gudang_tujuan != ""){
            $querydb->where('log_stock.to_gudang_id',$this->filter_gudang_tujuan);
        }else{
            $querydb->where('log_stock.to_gudang_id',0);
        }

        if($this->tgl_start != "" && $this->tgl_end !=""){
            $querydb->whereDate('log_stock.created_at','>=',date('Y-m-d',strtotime($this->tgl_start)));
            $querydb->whereDate('log_stock.created_at','<=',date('Y-m-d',strtotime($this->tgl_end)));
           
        }

        $data = $querydb->get();
        foreach($data as $key=> $value)
        {
            $value->no                = $key+1;
            $value->id                = $value->id;
            $value->nama_produk       = $value->product_name;
            $value->dari_stock        = $value->from_stock.' '.$value->name;
            $value->ke_stock          = $value->to_stock.' '.$value->name;
            $value->new_stock         = ($value->from_stock - $value->to_stock).' '.$value->name;
            $value->tgl_mutasi        = date('d-m-Y H:i',strtotime($value->created_at));
            $value->created_by        = $value->created_by;
        }
        
        return view('backend/stok/stokmutasihistory/excel', [
                'data' => $data,
                'perusahaan' => $perusahaan,
                'gudang' => $gudang,
                'gudang_tujuan' => $gudangtujuan,
                'tgl_start' => $this->tgl_start,
                'tgl_end' => $this->tgl_end,
                'tgl' => date('d/m/Y H:s')
        ]);
    }
}
