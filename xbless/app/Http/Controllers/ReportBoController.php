...
use App\Exports\ReportBOExports;
...
class ReportBoController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('backend/report/back_order/index_bo', compact('selectedfilterkeyword', 'kategori', 'selectedkategori', 'status', 'selectedfilterstatus', 'tgl_start', 'tgl_end', 'product'));
    }
    public function getData(Request $request)
    {
        ...
        foreach ($data as $key => $value) {
            ...
            $value->no_transaksi      = $value->flag_status == 0 ? $value->kode_rpo . '<br>' . '<span class="badge badge-info">' . $value->no_nota . '</span>' : $value->kode_rpo;
        ...
        if ($request->user()->can('reportbo.index')) {
            $json_data = array(
    ...
    public function print(Request $request)
    {
        ...
        foreach ($data as $key => $value) {
            ...
            $value->no_transaksi      = $value->flag_status == 0 ? $value->kode_rpo . '<br>' .  $value->no_nota : $value->kode_rpo;
            $value->status            = $value->flag_status == 0 ? 'Terkirim' : 'Belum Terkirim';
            $value->tgl               = date("d-m-Y", strtotime($value->dataorder));
            $value->created_by        = $value->created_by;
        }
        return view('backend/report/back_order/index_bo_print', compact('data', 'filter_status', 'filter_kategori', 'filter_keyword', 'filter_tgl_start', 'filter_tgl_end'));
    }
    public function pdf(Request $request)
    {
        ...
        $pdf = PDF::loadView('backend/report/back_order/index_bo_pdf', ['data' => $data, 'filter_status' => $filter_status, 'filter_kategori' => $filter_kategori, 'filter_keyword' => $filter_keyword, 'filter_tgl_start'   => $filter_tgl_start, 'filter_tgl_end'   => $filter_tgl_end], [], $config);
        ob_get_clean();
        return $pdf->stream('Report BO"' . date('d_m_Y H_i_s') . '".pdf');
    }
    public function excel(Request $request)
    {
        ...
        return Excel::download(new ReportBOExports($filter_kategori, $filter_status, $filter_keyword, $filter_tgl_start, $filter_tgl_end), 'Report BO.xlsx');
    }
}
