	        ...
	        <tbody>
	            @foreach($detail_tanda_terima as $key => $value)
					<tr>
                		...
                		<td align='right' style='font-size:13px;'>Rp. {{ number_format($value->nilai, 0, '', '.').'' }}</td>
	            	    <td align='right' style='font-size:13px;'>Rp. {{ number_format($value->nilai, 0, '', '.').'' }}</td>
	            	    <td></td>
	            	</tr>
				@endforeach
	            <tr height='30px'>
	            	...
	            	<td align="right" style="height: 15px;"><p style="font-size:14px;"><b>Rp. {{ number_format($tanda_terima->grandtotal, 0, '', '.').'' }}</b></p></td>
	            </tr>
	        </tbody>
	    </table>
	    <table border="0" width="100%">
			<tr>
                <th align="left">
                    <table style="border: 1px solid black; padding:25px;">
                        <tr>
                            <td><b>Nama</b></td>
                            <td>: <b>{{ $tanda_terima->getPerusahaan->name }}</b> </td>
                        </tr>
                        <tr>
                            <td><b>A/C</b></td>
                            <td>: <b>{{ $tanda_terima->getPerusahaan->rek_no }}</b></td>
                        </tr>
                        <tr>
                            <td><b>Bank</b></td>
                            <td>: <b>{{ $tanda_terima->getPerusahaan->bank_name }}</b></td>
                        </tr>
                    </table>
                </th>
	    		<th align="right" width="253px">
	    			<table width="253px">
	    				...
	    				<tr>
	    					<td align="center"><h5>{{ $tanda_terima->member_name }}</h5><br><br></td>
						</tr>
						<tr>
