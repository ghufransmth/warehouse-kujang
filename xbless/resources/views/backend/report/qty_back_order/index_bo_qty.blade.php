...
@section('title', 'LAPORAN BACK ORDER QTY')
@section('content')
<style>
.swal2-container{
...
<div class="row wrapper border-bottom white-bg page-heading">
    <div class="col-lg-10">
        <h2>LAPORAN BACK ORDER QTY</h2>
        <ol class="breadcrumb">
            <li class="breadcrumb-item">
                <a href="{{route('manage.beranda')}}">Beranda</a>
            </li>
            <li class="breadcrumb-item active">
                <a>LAPORAN BACK ORDER QTY</a>
            </li>
        </ol>
    </div>
    <div class="col-lg-2">
    ...
    var table,tabledata,table_index,tableproduct;
       $(document).ready(function(){
        ...
        $('#ExportExcel').on('click', function() {
            ...
            var jumlahdata = table.rows().count();
            if(jumlahdata > 0){
                @cannot('reportboqty.excel')
                    Swal.fire('Ups!', "Anda tidak memiliki HAK AKSES! Hubungi ADMIN Anda.",'error'); return false;
                @else
                    window.open('{{route('reportboqty.excel')}}', '_blank');
                @endcannot
            }else{
                Swal.fire('Ups','Tidak ada data','info');
                return false;
        ...
        $('#ExportPdf').on('click', function() {
            var jumlahdata = table.rows().count();
            if(jumlahdata > 0){
                @cannot('reportboqty.pdf')
                    Swal.fire('Ups!', "Anda tidak memiliki HAK AKSES! Hubungi ADMIN Anda.",'error'); return false;
                @else
                    window.open('{{route('reportboqty.pdf')}}', '_blank');
                @endcannot
            }else{
                Swal.fire('Ups','Tidak ada data','info');
                return false;
